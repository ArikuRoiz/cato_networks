...
def _build_messages(inp: JudgeInput) -> list[LLMMessage]:
    ...
    user = (
        ...
        f"Evidence: {evidence_line(inp.evidence)}\n"
        f"Technical: {technical_line(inp.technical_signal)}\n"
        f"Research plan (debate outcome): {research_plan_line(inp.research_plan)}\n"
        f"PM decision: {proposal_line(inp.trade_proposal)}\n"
        f"Cycle outcome: {inp.cycle_outcome or 'unknown'}\n"
        f"Synthesis summary: {synthesis_line(inp.synthesis)}\n\n"
        f"{_RUBRIC}\n"
